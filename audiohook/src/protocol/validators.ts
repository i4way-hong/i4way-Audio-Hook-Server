...
export const isEventEntities = (value: JsonValue): value is EventEntities & JsonObject => (
    Array.isArray(value) && value.every(isEventEntity)
);


export const isCloseReason = makeStringUnionValidator<CloseReason>({ 'disconnect': true, 'end': true, 'error': true, 'reconnect': true });
